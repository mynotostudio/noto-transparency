...
- **Trigger:** You generate practice exercises from a note.
- **User data sent:** The note text (HTML stripped, up to ~12,000 chars, including overflow child chunks and any backing document text) + subject + difficulty + question count (1–10) + exercise type (mixed / open / problems / math). Generated exercises are returned to you (not stored by Noto until you save an attempt).
- **Provider:** AI provider (model: "automatic").
- **Why:** To produce practice questions tailored to your material.
- **Provider retains:** TO BE VERIFIED.
- **Used for training:** TO BE VERIFIED.
- **Processing location:** Provider cloud — TO BE VERIFIED.
- **User control:** Don't generate exercises.

### Exercise grading

- **Trigger:** You submit your answers to a generated exercise set for grading.
- **User data sent:** The exercise questions + expected answers + your answers + your language. Grading results (per-exercise status, score, feedback, model answer) are returned to you and saved as an ExerciseAttempt when you finish.
- **Provider:** AI provider (model: "automatic").
- **Why:** To assess your answers against the model solutions.
- **Provider retains:** TO BE VERIFIED.
- **Used for training:** TO BE VERIFIED.
- **Processing location:** Provider cloud — TO BE VERIFIED.
- **User control:** Don't submit answers for grading.

### Grammar check

- **Trigger:** You run the grammar checker on a note (or a selection).
- **User data sent:** The text being checked (up to ~20,000 chars) + the detected note language. Sent to the **LanguageTool** public API (`api.languagetool.org`) — a dedicated grammar-checking service, not one of the LLM providers above. Matches (offset, length, message, suggested replacements, category) are returned to the editor.
- **Provider:** LanguageTool (free public API, no API key).
- **Why:** To surface spelling and grammar issues with inline suggestions.
- **Provider retains:** TO BE VERIFIED (per LanguageTool's privacy policy).
- **Used for training:** TO BE VERIFIED.
- **Processing location:** LanguageTool cloud (EU) — TO BE VERIFIED.
- **User control:** Don't run the grammar checker.

### Voice transcription (voice memos)

- **Trigger:** You record a voice memo in the editor and submit it for transcription.
- **User data sent:** The uploaded audio file URL (you upload the file first via `UploadPublicFile`, then only the URL is passed on). Uses the Base44 Core `TranscribeAudio` integration, backed by **OpenAI Whisper**. The returned transcript is inserted into your note as text.
- **Provider:** OpenAI (Whisper speech-to-text).
- **Why:** To turn spoken notes into editable text.
- **Provider retains:** TO BE VERIFIED.
- **Used for training:** TO BE VERIFIED.
- **Processing location:** OpenAI cloud — TO BE VERIFIED.
- **User control:** Don't record voice memos; delete the uploaded audio file.

### Rephrase & explain (Underline Ask)

- **Trigger:** You highlight a passage in a note and choose an action: explain, simplify, example, why, deep dive, analogy, quiz, correct, rephrase, fact-check, or ask a follow-up.
- **User data sent:** The selected text (up to ~6,000 chars) + surrounding context (text immediately before/after, up to ~600 chars each; the section heading; the containing block, up to ~800 chars) + note metadata (title, notebook, source type, key terms, AI summary) + conversation history (up to 8 turns for follow-up questions) + your language + the chosen action. The reply is returned to you (and can be applied back into the note for rephrase/correct).
- **Provider:** AI provider (model: "automatic").
- **Why:** To give instant, grounded explanations and rewrites of a specific passage.
- **Provider retains:** TO BE VERIFIED.
- **Used for training:** TO BE VERIFIED.
- **Processing location:** Provider cloud — TO BE VERIFIED.
- **User control:** Don't use the underline-ask feature.
